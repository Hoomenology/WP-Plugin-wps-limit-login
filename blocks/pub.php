...
$datas = \WPS\WPS_Limit_Login\Pub::get_api_result();

if ( empty( $datas ) ) {
	return false;
}

$plugins_adds = \WPS\WPS_Limit_Login\Pub::get_json_array( $datas );
...
foreach ( $plugins_adds as $add ) {
	...
	if ( ! \WPS\WPS_Limit_Login\Pub::is_plugin_installed( $plugin ) ) {
